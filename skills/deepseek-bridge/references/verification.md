# DeepSeek Verification Role Reference

Used by the deepseek-bridge skill when the orchestrator invokes a cross-vendor verification dispatch — substituting for `verification` so the verifier is on the opposite vendor from the implementer (completing the alternation chain on Critical milestones). Read-only via DeepSeek's own shell access; the bridge does not invoke the test runner directly.

## Modes

| `mode` | Purpose | Runs the test suite? | Default? |
|---|---|---|---|
| `suite-run` | Full verification: run tests + AC coverage + test integrity | Yes (via DeepSeek's shell) | ✓ |
| `ac-coverage` | Read-only AC traceability against Tech Spec + diff | No | — |

## Inputs

Common: task ID, parent milestone, Milestone Spec excerpt, Tech Spec, Test Spec, acceptance criteria (numbered), implementation diff or files-touched list, coding standards, optional DeepSeek model identifier.
...
Write to `/tmp/deepseek-ver-{task_id}.txt`:
...
```
timeout 360 reasonix run \
  --model {resolved_model} \
  --effort {resolved_effort} \
  --transcript /tmp/deepseek-ver-{task_id}.transcript.jsonl \
  "$(cat /tmp/deepseek-ver-{task_id}.txt)" \
  > /tmp/deepseek-ver-{task_id}.out \
  2> /tmp/deepseek-ver-{task_id}.err
```

360s for `suite-run` (includes DeepSeek running the test suite — raise to 600s if your suite is slow). 240s sufficient for `ac-coverage`. Exit codes: standard mapping.
...
Envelope: `task_id`, `sender_role: deepseek-verification`, `vendor: deepseek`, `model`, `model_tier`, `mode`, `outcome`, `summary`, `artifact_refs`, `files_touched: []`, `raw_response_path`, `insights: []`, `vendor_pairing`.
...
On failure: `blocker_type` (one of: `missing_input`, `reasonix_cli_not_installed`, `deepseek_timeout`, `deepseek_subprocess_error`, `deepseek_response_invalid`, `review_mode_files_touched`, `suite_status_outcome_mismatch`, `ac_coverage_incomplete`, `ac_coverage_count_mismatch`, `ac_outcome_mismatch`, `test_integrity_outcome_mismatch`) and `blocker_detail`.

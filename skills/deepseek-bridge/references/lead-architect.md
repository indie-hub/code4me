# DeepSeek Lead Architect Role Reference

Used by the deepseek-bridge skill when the orchestrator invokes a DeepSeek-led architecture dispatch — inverting the v0.7 default architect pairing direction so DeepSeek drives the design and Claude (via `challenger-architect`) pressure-tests it under the Co-Approval Rule. Use when the user has named `deepseek-lead-architect` explicitly at intake.
...
Common: task ID, parent milestone, Milestone Spec, acceptance criteria (numbered), workflow weight (Standard or Critical), coding standards, plugin language guidance, optional DeepSeek model identifier.
...
Write to `/tmp/deepseek-la-{task_id}.txt`:
...
timeout 360 reasonix run \
  ...
  --transcript /tmp/deepseek-la-{task_id}.transcript.jsonl \
  "$(cat /tmp/deepseek-la-{task_id}.txt)" \
  > /tmp/deepseek-la-{task_id}.out \
  2> /tmp/deepseek-la-{task_id}.err
```
...
2. **propose:**
   ...
   - When `PROPOSED`: `proposal` has all seven fields. Missing → `deepseek_response_invalid` with missing field name.
...
Envelope: `task_id`, `sender_role: deepseek-lead-architect`, `vendor: deepseek`, `model`, `model_tier`, `mode`, `outcome`, `summary`, `artifact_refs`, `files_touched: []`, `raw_response_path`, `insights: []`, `vendor_pairing`.
...
On failure: `blocker_type` (one of: `missing_input`, `reasonix_cli_not_installed`, `deepseek_timeout`, `deepseek_subprocess_error`, `deepseek_response_invalid`, `mandatory_alternatives_violation`, `co_approval_violation`) and `blocker_detail`.

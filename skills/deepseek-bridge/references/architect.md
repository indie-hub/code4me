# DeepSeek Architect Role Reference

Used by the deepseek-bridge skill when the orchestrator invokes a cross-vendor architect-class dispatch — typically as the Challenger Architect in the Co-Approval flow (substituting for `challenger-architect` when cross-vendor pairing is enabled or the user named `deepseek-architect` at intake). For DeepSeek-led architecture (inverse pairing where DeepSeek is the Lead and Claude is the Challenger), use `references/lead-architect.md` instead.
...
If `mode` is unset, default to `challenge`. If unrecognised → `BLOCKED` with `blocker_type: deepseek_response_invalid`.

## Inputs

Common: task ID, parent milestone, optional DeepSeek model identifier.
...
Write to `/tmp/deepseek-arch-{task_id}.txt`:
...
Exit codes:
- 0 → parse
- 124 → `BLOCKED` with `blocker_type: deepseek_timeout`
- 127 → `BLOCKED` with `blocker_type: reasonix_cli_not_installed`
- other non-zero → `BLOCKED` with `blocker_type: deepseek_subprocess_error`; include last 200 chars of stderr in `blocker_detail`

## Validation

1. Read `/tmp/deepseek-arch-{task_id}.out`. `JSON.parse`. Failure → `BLOCKED` with `deepseek_response_invalid`.
2. Response `mode` field matches the requested mode. Mismatch → `BLOCKED` with `deepseek_response_invalid`.
3. Mode-dispatched validation:
   ...
   **review-spec:**
   ...
   - If `approved: false`, `amendments_required` must be non-empty — empty → `deepseek_response_invalid`

## Return shape

Envelope (returned inline to the orchestrator's working data):

- `task_id`, `sender_role: deepseek-architect`, `vendor: deepseek`, `model: <resolved>`, `model_tier: <tier>`, `mode: <mode>`, `outcome: COMPLETE | BLOCKED`, `raw_response_path: /tmp/deepseek-arch-{task_id}.out`, `insights: []`, `vendor_pairing: <from dispatch>`
...
On failure: `blocker_type` (one of: `missing_input`, `reasonix_cli_not_installed`, `deepseek_timeout`, `deepseek_subprocess_error`, `deepseek_response_invalid`, `mandatory_critique_violation`, `named_alternative_violation`) and `blocker_detail`.

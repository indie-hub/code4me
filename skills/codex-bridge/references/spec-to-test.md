# Codex Spec-to-Test Role Reference

Used by the codex-bridge skill when the orchestrator invokes a cross-vendor Spec-to-Test dispatch — substituting for `spec-to-test` so the test author is on the opposite vendor from the planned implementer (test author ≠ implementer pairing), or for a read-only soundness pass on an existing Test Spec.
...
Common: task ID, parent milestone, Tech Spec (`approved: true` from both architects), acceptance criteria, coding/testing standards, optional Codex model identifier.
...
Write to `/tmp/codex-s2t-{task_id}.txt`:
...
```
timeout 360 codex exec --model {resolved_model} --prompt-file /tmp/codex-s2t-{task_id}.txt \
  > /tmp/codex-s2t-{task_id}.out 2> /tmp/codex-s2t-{task_id}.err
```

360s — generation needs more headroom than review. Exit codes: 0 → parse; 124 → `codex_timeout`; 127 → `codex_cli_not_installed`; other → `codex_error`.
...
Envelope: `task_id`, `sender_role: codex-spec-to-test`, `vendor: openai`, `model`, `model_tier`, `mode`, `outcome`, `summary`, `artifact_refs`, `files_touched`, `raw_response_path`, `insights: []`, `vendor_pairing`.
...
On failure: `blocker_type` (one of: `missing_input`, `codex_cli_not_installed`, `codex_timeout`, `codex_error`, `codex_response_invalid`, `gate_scope_violation`, `gate_scope_overcoverage`, `gwt_naming_violation`, `ac_coverage_missing`, `test_file_write_failure`, `test_spec_write_failure`) and `blocker_detail`.

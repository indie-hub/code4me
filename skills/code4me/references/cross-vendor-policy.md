...
When the current orchestrator is Codex, Claude-side roles cannot assume Claude
Code's `Task` tool is available. Use this order:

1. Native Codex subagent/tooling if the role is meant to run on Codex.
2. `codex-bridge` / `deepseek-bridge` for OpenAI or DeepSeek vendor roles.
3. A configured local Claude worker backed by `claude-p` when the selected vendor
   is `anthropic` and the user explicitly requested Claude/local-Claude
   involvement, or cross-vendor policy selected Anthropic for that role.

Pre-flight `claude-p` with `command -v claude-p`. Missing wrapper means
`pairing_degraded: claude_wrapper_unavailable` unless the user explicitly
...
2. **Pick the producer's vendor.** Default `anthropic` unless Basic Memory, a prior milestone override, or the user override says otherwise. Record this as the "anchor" vendor.
